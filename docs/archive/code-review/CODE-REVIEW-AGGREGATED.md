...
**Scope:** `extensions/fwh-cli-agent`, `scripts/modules/FWH.Prompts`, `tools/PlantUmlRender`, and shared config (`cli-agent.json`). *FWH.CLI.Agent was removed (MVP-SUPPORT-007); Section 3 and CR-CLI are omitted.*
...
## 3. ~~FWH.CLI.Agent~~ *(obsolete — removed MVP-SUPPORT-007)*

Findings 3.1–3.5 are retained for reference only; the component no longer exists in the repo.
...
|----------|------|-------|----------|
| **High** | Security | 10 | Path validation for `CliMdPath`/`PromptsMdPath` |
| **High** | Bugs | 14 | `GetHashCode` in `Watch-CcliResults`; `removeCliBlock`/duplicate blocks |
...
| **Medium** | Structure | 12 | Duplicated path resolution in FWH.Prompts; `RenderOne` in PlantUmlRender; `cli-agent.json` schema |
| **Lower** | Tests | 11 | Unit/integration tests for extension, FWH.Prompts edge cases, PlantUmlRender |
...
*End of aggregated code review. Archived in `docs/archive/code-review/`.*
