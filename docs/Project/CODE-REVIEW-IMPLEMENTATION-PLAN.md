...
**Components:** EXT = extensions/fwh-cli-agent, PSM = scripts/modules/FWH.Prompts, CLI = src/FWH.CLI.Agent *(removed per MVP-SUPPORT-007; CR-CLI / Agent CLI items are obsolete)*, PUM = tools/PlantUmlRender, CFG = cli-agent.json / cross-cutting.
...
- **One agent per component** (EXT, PSM, CLI, PUM, CFG). Each agent may edit only the paths in its roster. This avoids merge conflicts and allows parallel work.
- **Respect dependency order** within an agent’s batch: e.g. CR-PSM-2.1.1 before CR-PSM-2.1.3; CR-CLI-3.1.4 before CR-CLI-3.2.3; CR-CLI-3.4.2 before CR-CLI-3.1.3.
...
- **Agent CFG** has work only in Phase 4 (schema + doc). It can run in parallel with EXT, PSM, CLI, PUM in Phase 4.
...
| **Agent CLI** | src/FWH.CLI.Agent | `src/FWH.CLI.Agent/**`, `tests/FWH.CLI.Agent.Tests/**` (create if needed) |
...
| Phase | Agent EXT | Agent PSM | Agent CLI | Agent PUM | Agent CFG |
|-------|-----------|-----------|-----------|-----------|-----------|
| **1** | 1.1.1, 1.1.3, 1.2.5 | 2.1.1 → 2.1.3, 2.2.3 | 3.1.1, 3.1.2, 3.1.4 → 3.2.3, 3.2.1, 3.2.2, 3.2.4 | 4.1.1 | — |
| **2** | 1.2.1, 1.2.2, 1.2.3, 1.2.4 | 2.2.1, 2.2.2, 2.2.4, 2.2.5 | 3.2.5, 3.2.6 | 4.2.1, 4.2.2, 4.2.3, 4.2.4 | — |
| **3** | 1.3.1, 1.3.2 | 2.3.1, 2.3.2 | 3.3.2, 3.3.3 | 4.3.1 | — |
| **4** | 1.4.1, 1.4.2, 1.4.3 | 2.1.2, 2.4.1, 2.4.2, 2.4.3, 2.4.4 | 3.4.1, 3.4.2 → 3.1.3, 3.4.3, 3.4.4 → 3.1.5, **5.1.2** | 4.1.2, 4.4.1, 4.4.2, 4.4.3 | **5.1.1** |
| **5** | 1.5.1, 1.5.2 | 2.5.1, 2.5.2, 2.5.3 | 3.5.1, 3.5.2, 3.5.3 | 4.5.1 | — |

*(→ means “after”: e.g. 2.1.1 → 2.1.3 = do 2.1.1 first. 5.1.2 is in Program.cs, so assigned to Agent CLI; 5.1.1 is schema/doc, assigned to Agent CFG.)*
...
References:
- [CODE-REVIEW-AGGREGATED.md](docs/Project/CODE-REVIEW-AGGREGATED.md) — full findings and actions
- [CODE-REVIEW-IMPLEMENTATION-PLAN.md](docs/Project/CODE-REVIEW-IMPLEMENTATION-PLAN.md) — phases, IDs, and agent matrix

For each CR-{Component}-{ID}, the Action column in the plan (and the matching review section) is the source of truth. After editing, run the relevant build/test for your component (e.g. `npm run compile` in the extension, `Invoke-Pester` for PSM, `dotnet build`/`dotnet test` for CLI/PUM).
```
...
**Do not edit:** `scripts/`, `src/FWH.CLI.Agent/`, `tools/PlantUmlRender/`, `cli-agent.schema.json`, `docs/Project/cli-agent-config.md`, or any path outside `extensions/fwh-cli-agent/`.
...
**Do not edit:** `extensions/`, `src/FWH.CLI.Agent/`, `tools/PlantUmlRender/`, `cli-agent.schema.json`, `docs/Project/cli-agent-config.md`, or any path outside `scripts/modules/FWH.Prompts/`.
...
### Agent Prompt: Agent CLI

*Copy-paste this into a new agent/chat.*

```
You are **Agent CLI**. Implement all code review items for **src/FWH.CLI.Agent** (and CR-CFG-5.1.2 in Program.cs) in Phases 1–5, in order. You may edit only `src/FWH.CLI.Agent/` and create/edit `tests/FWH.CLI.Agent.Tests/` if needed.

**Your CR- IDs in order (respect →):**
- Phase 1: CR-CLI-3.1.1, CR-CLI-3.1.2, CR-CLI-3.1.4, CR-CLI-3.2.1, CR-CLI-3.2.2, CR-CLI-3.2.4, then CR-CLI-3.2.3
- Phase 2: CR-CLI-3.2.5, CR-CLI-3.2.6
- Phase 3: CR-CLI-3.3.2, CR-CLI-3.3.3
- Phase 4: CR-CLI-3.4.1, CR-CLI-3.4.2, then CR-CLI-3.1.3, CR-CLI-3.4.3, CR-CLI-3.4.4, then CR-CLI-3.1.5, and CR-CFG-5.1.2 (remove or implement `_promptsMdPath` in Program.cs)
- Phase 5: CR-CLI-3.5.1, CR-CLI-3.5.2, CR-CLI-3.5.3

**Do not edit:** `extensions/`, `scripts/modules/FWH.Prompts/`, `tools/PlantUmlRender/`, `cli-agent.schema.json`, `docs/Project/cli-agent-config.md`. You may add `cli-agent.json` config keys only if required for new options (e.g. ReinitOnStart, RunTimeoutSeconds, AgentTimeoutMinutes, AgentPath); do not change schema or docs owned by Agent CFG.

Use CODE-REVIEW-AGGREGATED.md and CODE-REVIEW-IMPLEMENTATION-PLAN.md for the exact action for each ID. After implementation, run `dotnet build` and `dotnet test` for FWH.CLI.Agent and FWH.CLI.Agent.Tests.
```
...
**Do not edit:** `extensions/`, `scripts/modules/FWH.Prompts/`, `src/FWH.CLI.Agent/`, `cli-agent.schema.json`, `docs/Project/cli-agent-config.md`, or any path outside `tools/PlantUmlRender/` and `tests/PlantUmlRender.Tests/`.
...
*Copy-paste this into a new agent/chat. Agent CFG runs only in Phase 4; it can be started in parallel with EXT/PSM/CLI/PUM when they begin Phase 4.*
...
1. **Phases 1–3 and 5:** Start **Agent EXT**, **Agent PSM**, **Agent CLI**, and **Agent PUM** in four separate agent/chat sessions (or four Cursor composer/agent tabs). Give each the **Shared context** and its **Agent prompt** above. Each implements its phases in order. These four can run fully in parallel.
2. **Phase 4:** Start **Agent EXT**, **Agent PSM**, **Agent CLI**, **Agent PUM**, and **Agent CFG** in five sessions. All five can run in parallel. Agent CFG only does CR-CFG-5.1.1; the others do their Phase 4 IDs.
...
| CR-CLI-3.2.1 | CLI | 3.2.1 | `ExecuteRun`: in `catch (OperationCanceledException)` ensure both stdout/stderr are not left hanging; `process.Kill()` then document or `WaitForExit`. | — |
| CR-CLI-3.2.2 | CLI | 3.2.2 | `ExecuteRun`: await `stdoutTask` and `stderrTask` once, then `AppendLine(stdout)` and append stderr. | — |
| CR-CLI-3.2.4 | CLI | 3.2.4 | `Watcher.Changed`: wrap entire handler body in `try/catch`, log, and ensure no unobserved `Task`. | — |
...
### 2.3 FWH.CLI.Agent (CLI)

| ID | Component | Review # | Action | Deps |
|----|-----------|----------|--------|------|
| CR-CLI-3.2.5 | CLI | 3.2.5 | `ProcessCliFile`: add tests for malformed ```cli blocks; consider stricter regex or scanner for block boundaries. | — |
| CR-CLI-3.2.6 | CLI | 3.2.6 | `_projectRoot`: add `?? throw new InvalidOperationException("Project root not set.")` at top of methods that use it, or `!` with comment. | — |

### 2.4 PlantUmlRender (PUM)
...
| CR-CLI-3.3.2 | CLI | 3.3.2 | Make `ExecuteRun` timeout configurable (`CliAgent:RunTimeoutSeconds`) or increase and document. | — |
| CR-CLI-3.3.3 | CLI | 3.3.3 | Make `ExecutePrompt` agent timeout configurable (`CliAgent:AgentTimeoutMinutes`). | — |
...
### 4.3 FWH.CLI.Agent (CLI)

| ID | Component | Review # | Action | Deps |
|----|-----------|----------|--------|------|
| CR-CLI-3.4.1 | CLI | 3.4.1 | Introduce `CliAgentService` (or similar) with `IConfiguration`, `ILogger`, paths in constructor; resolve from host; keep `Main` thin. | — |
| CR-CLI-3.4.2 | CLI | 3.4.2 | Extract `RunProcessAsync(ProcessStartInfo, ILogger?, TimeSpan?, CancellationToken)` returning (stdout, stderr, exitCode). Use in `ExecuteBuild`, `ExecuteTest`, `ExecuteClean`, `ExecuteDotNet`, `ExecuteGit`, `ExecutePowerShell`, `ExecuteShellCommand`. | — |
| CR-CLI-3.4.3 | CLI | 3.4.3 | Document that `ParseCommand` supports simple quoted strings; note limitations vs. full shell. | — |
| CR-CLI-3.4.4 | CLI | 3.4.4 | Document that `agent` must be on `PATH` on all platforms; optionally add `CliAgent:AgentPath`. | — |
| CR-CLI-3.1.5 | CLI | 3.1.5 | Document `agent` on PATH; optional `CliAgent:AgentPath`. | CR-CLI-3.4.4 |

### 4.4 PlantUmlRender (PUM)
...
| ID | Component | Review # | Action | Deps |
|----|-----------|----------|--------|------|
| CR-CFG-5.1.1 | CFG | 5.1.1 | Add `cli-agent.schema.json` and optional validation in extension, PSM, CLI; or document exact shape. Consider `ExecuteMode`/`ComposerCommand` in C# app if it drives Composer. | — |
| CR-CFG-5.1.2 | CFG | 5.1.2 | Remove `_promptsMdPath` and its config read in `Program.cs` if unused, or implement loading prompts from that path for `prompt`. | — |
...
### 4.8 FWH.CLI.Agent – ArgumentList

| ID | Component | Review # | Action | Deps |
|----|-----------|----------|--------|------|
| CR-CLI-3.1.3 | CLI | 3.1.3 | For `dotnet`/`git`: use `ProcessStartInfo.ArgumentList` instead of `Arguments = string.Join(" ", args)` where possible. | CR-CLI-3.4.2 |
...
### 5.3 FWH.CLI.Agent (CLI)

| ID | Component | Review # | Action | Deps |
|----|-----------|----------|--------|------|
| CR-CLI-3.5.1 | CLI | 3.5.1 | Add test project. Unit: `ParseCommand` (quotes, spaces, empty), `FindProjectRoot`, regex for ```cli and ## Results; `ExecuteClean`, `ExecuteStatus`, `ExecuteList` with temp dir. | — |
| CR-CLI-3.5.2 | CLI | 3.5.2 | Mock `Process.Start` and stdout/stderr for `ExecutePrompt` and `ExecuteShellCommand`; assert `ProcessStartInfo` and error handling. | — |
| CR-CLI-3.5.3 | CLI | 3.5.3 | Integration: temp dir with `FunWasHad.sln` and `cli-agent.json`; `CreateInitialCliFile`, `ProcessCliFile`, `ExecuteCleanCli`; assert `CLI.md` and `CLI-history.md`. | — |

### 5.4 PlantUmlRender (PUM)
...
- **Phase 3** (performance) is largely independent; can overlap with Phase 2. All four component agents (EXT, PSM, CLI, PUM) can run in parallel.
- **Phase 4** (structure): CR-PSM-2.4.1 benefits from CR-PSM-2.1.1 (done in P1); CR-CLI-3.4.2 enables cleaner CR-CLI-3.1.3. **Agent CFG** joins in Phase 4 only; all five agents can run in parallel.
- **Phase 5** (tests) can run in parallel across EXT, PSM, CLI, PUM; prefer to add tests after the code under test is stable.
